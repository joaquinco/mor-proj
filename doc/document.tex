...
\begin{document}
  ...
  La representación del problema en computadora es sencilla, se requiere:
  ...
  \begin{itemize}
    ...
    \item Un parámetro $\rho$ de penalización del arribo a los clientes luego del valor $l_i$.
  \end{itemize}
  ...
  Esta fase, representada por la función $ConstructSolution(instance)$ en el pseudocódigo (\ref{grasp}) construye una solución secuencialmente partiendo de una solución vacía. En cada iteración decide cuál cliente satisfacer por cuál vehículo de esta manera construyendo potencialmente varias rutas de manera simultánea.
  
  En cada iteración, se toman los $MovesPerVehicle$ mejores movimientos posibles de cada vehículo y se agregan a una lista, luego se construye la RCL a partir de esa lista. Vale la pena aclarar que se consideran todos los vehículos, incluso los que no tienen clientes asignados y se asume que se encuentran en el nodo origen.

  Los mejores movimientos son determinado por su costo. El costo de un movimiento depende del nodo actual en que se encuentra el vehículo, el nodo destino y el tiempo actual. De esta manera se computa el costo como:
  ...
  \begin{description}
    ...
    \item[$PT_{ijT}$:] Es la proximidad del arribo a $j$ al momento de cierre. Es decir $max(l_j - a_{vj}, 0)$.
    \item[$WT_{ijT}$]: Es el tiempo de espera hasta la apertura de $j$. Es decir $max(e_j - a_{vj}, 0)$.
